...
class SessionBuilder {
    ...
    async initSession(isInitiator, ourEphemeralKey, ourSignedKey, theirIdentityPubKey,
        ...
        let sharedSecret;
        if (native && typeof native.buildSessionSharedSecretAsync === 'function') {
            sharedSecret = await native.buildSessionSharedSecretAsync(isInitiator, a1, a2, a3, a4);
        } else if (native && typeof native.buildSessionSharedSecret === 'function') {
            sharedSecret = native.buildSessionSharedSecret(isInitiator, a1, a2, a3, a4);
        } else {
            if (!a4) {
